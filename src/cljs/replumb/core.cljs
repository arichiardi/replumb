...
(defn ^:export read-eval-call
  ...
  * `:write-file-fn!` a synchronous 2-arity function that accepts a file-path
  and data to write.
...
(defn ^:export browser-options
  ...
  a synchronous 2-arity function of signature `[file-path data]`."
  ([load-fn!]
   {:target :default
    :load-fn! load-fn!})
