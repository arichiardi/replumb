...
"]
  ...
  (deftest context-value
    (let [res (read-eval-call "2 3")
      ...
      (is (valid-eval-error? error) "\"2 3\" with :context set to :expr should be an instance of jsError")bcor
      (is (re-find #"ERROR - number is not a function" (extract-message error)) "\"2 3\" with :context set to :expr should have a valid error message."))
